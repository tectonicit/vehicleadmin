...
export default function DataGridBooked() {
  ...
  const fetchBookedVehicle = async () => {
    try {
     ...
     const response = await axios.get(`${baseUrl}bookedvehicles`);
  ...
  const updateRecord = async (data) => {
    ...
    try {
      ...
      const response = await axios.put(`${baseUrl}bookedvehicle/release`, data, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
  ...
  //   { id, name: '', driveraddress:"",mobile:"", status: '', isNew: true },
  const columns = [
    { field: 'booked_id', headerName: 'Booked ID', width: 180, editable:false, headerClassName: 'header-row' },
    {
      field: 'driver_Id',
